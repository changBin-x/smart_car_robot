...
- **可选 USB 单目相机**：`use_hik_camera:=true` 时包含 `hik_camera_bringup`；`usb_cam` 在内部采集原始 MJPEG，`hik_mjpeg_decoder_node` 校验并解码为对外的 `/hik_monocular/image_raw`（`bgr8`）。相机和 `web_video_server` 预览默认关闭，避免未接硬件或预览负载影响控制栈。
