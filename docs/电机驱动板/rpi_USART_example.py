...
def send_data(data):
    ser.write(
        data.encode()
    )  # 将字符串转换为字节后发送    Convert the string to bytes before sending
    time.sleep(
        0.01
    )  # 延时确保数据发送完成    Delay to ensure data transmission is completed
...
def receive_data():
    ...
    if (
        ser.in_waiting > 0
    ):  # 检查串口缓冲区是否有数据  Check if there is data in the serial port buffer
        recv_buffer += ser.read(
            ser.in_waiting
        ).decode()  # 读取并解码数据  Read and decode data
...
def parse_data(data):
    data = (
        data.strip()
    )  # 去掉两端的空格或换行符   Remove spaces or line breaks at both ends

    if data.startswith("$MAll:"):
        values_str = data[6:-1]  # 去除 "$MAll:" 和 "#" Remove "$MAll:" and "#"
        values = list(
            map(int, values_str.split(','))
        )  # 分割并转换为整数  Split and convert to integer
    ...
    try:
        ...
        while True:
            ...
            if MOTOR_TYPE == 4:
                ...
                control_speed(
                    M1, M2, M3, M4
                )  # 直接发送命令控制电机  Send commands directly to control the motor
