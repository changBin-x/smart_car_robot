...
基于 **React 18 + Vite + Material Design 3 (MD3) + Three.js + Node.js** 的四轮麦克纳姆轮全向智能小车上位机控制与监控系统。支持通过 `rosbridge_websocket` (端口 `9090`) 远程连接树莓派 4B (`192.168.10.11`)，实现高频姿态遥测、电池电量精准估算、高德地图轨迹追踪、全向运动控制、指令调试控制台与历史数据持久化查询。
...
- **npm**: `>= 9.0.0`
- **ROS 2 主机**: 树莓派 4B (`192.168.10.11`) 运行 ROS 2 Jazzy 并开启 `rosbridge_websocket` (端口 `9090`)
